    ...
    <v-stepper-items>
      ...
      <v-stepper-content step="4">
        <v-flex class="mx-auto">
          <!--遍历特有规格参数-->
          <v-card flat  v-if="specialSpecs !== null && specialSpecs.length > 0"   v-for="spec in specialSpecs" :key="spec.name">
            <!--特有参数的标题-->
            <div class="subheading">{{spec.name}}:</div>
            <!--特有参数的待选项，需要判断是否有options，如果没有，展示文本框，让用户自己输入-->
            <v-card-text class="px-5">
              <div  v-if="spec.options !== null && spec.options.length > 0"     v-for="i in spec.options.length+1" :key="i" class="layout row px-5">
